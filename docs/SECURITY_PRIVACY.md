...
- **What is safe to cache:** public route content only — sanitized schedule anchors, app-safe dossiers, tokens/fonts/icons, and the public shell. Field notes are not service-worker cached: they live in localStorage first, and team sync (when enabled and signed in) uses cross-origin Supabase REST under the user's own session.
